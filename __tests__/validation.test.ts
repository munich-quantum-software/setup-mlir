...
describe("Version Validation", () => {
  ...
  it("should recognize invalid commit hashes", () => {
    ...
    expect(commitRegex.test("12345678901234567890123456789012345678901")).toBe(
      false,
    ); // too long
  });
});
...
describe("Asset Name Pattern Matching", () => {
  ...
  it("should match platform-specific asset patterns", () => {
    // Linux X86
    expect(
      /.*_linux_.*_X86\.tar\.zst$/i.test(
        "llvm-mlir_llvmorg-21.1.8_linux_ubuntu-24.04_X86.tar.zst",
      ),
    ).toBe(true);
    // Linux ARM
    expect(
      /.*_linux_.*_AArch64\.tar\.zst$/i.test(
        "llvm-mlir_llvmorg-21.1.8_linux_ubuntu-24.04_AArch64.tar.zst",
      ),
    ).toBe(true);
    // macOS X86
    expect(
      /.*_macos_.*_X86\.tar\.zst$/i.test(
        "llvm-mlir_llvmorg-21.1.8_macos_macos-15_X86.tar.zst",
      ),
    ).toBe(true);
    // macOS ARM
    expect(
      /.*_macos_.*_AArch64\.tar\.zst$/i.test(
        "llvm-mlir_llvmorg-21.1.8_macos_macos-14_AArch64.tar.zst",
      ),
    ).toBe(true);
    // Windows X86
    expect(
      /.*_windows_.*_X86\.tar\.zst$/i.test(
        "llvm-mlir_llvmorg-21.1.8_windows_windows-2022_X86.tar.zst",
      ),
    ).toBe(true);
    // Windows ARM
    expect(
      /.*_windows_.*_AArch64\.tar\.zst$/i.test(
        "llvm-mlir_llvmorg-21.1.8_windows_windows-11_AArch64.tar.zst",
      ),
    ).toBe(true);
  });
});
