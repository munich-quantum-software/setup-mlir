...
```yaml
- name: Setup MLIR
  uses: munich-quantum-software/setup-mlir@v1.0.0
  with:
    llvm-version: 21.1.8
```
...
The scripts will use `tar` with native zstd support if available, otherwise `zstd` needs to be installed on the host system.
